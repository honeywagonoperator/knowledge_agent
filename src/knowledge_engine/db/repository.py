...
class Repository[T]:
    ...
    async def get(self, id: UUID | str) -> T | None: return await self._session.get(self._model, id)
    async def list(self, skip: int = 0, limit: int = 100) -> Sequence[T]:
        from sqlalchemy import select; stmt = select(self._model).offset(skip).limit(limit)
        result = await self._session.execute(stmt); return result.scalars().all()
    async def count(self) -> int:
        stmt = select(func.count()).select_from(self._model)
        result = await self._session.execute(stmt); return result.scalar_one()
