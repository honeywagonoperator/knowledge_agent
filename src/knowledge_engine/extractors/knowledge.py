...
class KnowledgeExtractor:
    def __init__(self, session: AsyncSession) -> None:
        ...
        self._doc_repo = Repository(session, Document)
        self._unit_repo = Repository(session, KnowledgeUnit)
    ...
    async def extract_from_source(self, source_id: UUID) -> int:
        ...
        for doc in documents:
            total += await self._extract_from_document(doc)
        return total
    ...
    async def _create_knowledge_unit(
        self, document: Document, chunk: str
    ) -> KnowledgeUnit | None:
        # Generate embedding via LlamaIndex (handled in vector step later)
        domain = self._guess_domain(chunk)
    ...
    def _guess_domain(self, text: str) -> str:
        keywords = {
            "programming": ["python", "code", "api", "function", "class", "library"],
            "ai-ml": ["neural", "model", "training", "llm", "embedding", "rag"],
            "data-engineering": ["pipeline", "database", "etl", "sql", "schema"],
            "devops": ["deploy", "docker", "ci/cd", "pipeline", "test"],
        }
